const ResponAnalyseModel = require("../models/respon-analysemodel");
...
module.exports = {
	register: async (req, res) => {
		...
		const responAnalyse = new ResponAnalyseModel({
		...
		let user = await ResponAnalyseModel.findOne({ email: responAnalyse.email });
		if (user) {
			return res
				.status(201)
				.json({ message: "Cet utilisateur est deja existe!" });
		}
		responAnalyse.save(req.body, (err, item) => {
			if (err) {
				res
					.status(406)
	...
	getall: async (req, res) => {
		await ResponAnalyseModel.find({}).exec((err, items) => {
			if (err) {
				res.status(406).json({
					success: false,
	...
	getbyid: async (req, res) => {
		await ResponAnalyseModel.findById(req.params.id).exec((err, item) => {
			if (err) {
				res.status(406).json({
					success: false,
					message: "Failed to get responsable analyse",
	...
	update: async (req, res) => {
		await ResponAnalyseModel.findByIdAndUpdate(req.params.id, req.body, {
			new: true,
		}).exec((err, item) => {
			if (err) {
				res.status(406).json({
					success: false,
					message: "Failed to update responsable analyse",
					data: null,
					err,
				});
			} else {
				res.status(201).json({
					success: true,
					message: " responsable analyse updated successufuly",
					data: item,
				});
			}
			//pour quitter la fonction async
	...
	delete: async (req, res) => {
		await ResponAnalyseModel.findByIdAndRemove(req.params.id).exec(
			(err, item) => {
				if (err) {
					res.status(406).json({
						success: false,
						message: "Failed to delete respnsable analyse",
						data: null,
						err,
					});
				} else {
					res.status(201).json({
						success: true,
						message: "responsable analyse deleted successufuly",
						data: item,
					});
				}
				//pour quitter la fonction async
