...
export default function ({
  ...
  const list = [`Nom: ${JSON.stringify(item.item.nompatient)}`,`Prénom: ${JSON.stringify(item.item.prenompatient)}`,`Nom Medecin: ${JSON.stringify(item.item.nommedecin)}` ,`Prénom Medecin : ${JSON.stringify(item.item.prenommedecin)}`,`Date: ${JSON.stringify(item.item.date_certificat)}`,`Email: ${JSON.stringify(item.item.emailpatient)}`, `Document: ${JSON.stringify(item.item.path)}`]
  ...
  return (
    <Layout>
      <TopNav
        middleContent="Resultat Radiologie"
        rightContent={
          <Ionicons
